...
class DinoVisionTransformer(nn.Module):
    ...
    def forward_features_list(
        ...
        topo_levels: Optional[Dict[str, Tensor]] = None,
        ...
        topo_hook = getattr(self, "_topo_hook", None)
        for block_idx, blk in enumerate(self.blocks):
            ...
            # Optional topology conditioning hook.
            # Expected signature: topo_hook(block_idx: int, x_list: List[Tensor], topo_levels: Optional[Dict[str, Tensor]]) -> List[Tensor]
            if topo_hook is not None and topo_levels is not None:
                x = topo_hook(int(block_idx), x, topo_levels)

            x = blk(x, rope_sincos)

            # Optional post-block topology conditioning hook.
            # If present, this enables stage-end CTI (e.g. ViT-CoMer style).
            if topo_hook is not None and topo_levels is not None and hasattr(topo_hook, "post_block"):
                x = topo_hook.post_block(int(block_idx), x, topo_levels)
        all_x = x
        output = []
    ...
    def forward_features(
        ...
        if isinstance(x, torch.Tensor):
            return self.forward_features_list([x], [masks], topo_levels=topo_levels)[0]
        else:
            return self.forward_features_list(x, masks, topo_levels=topo_levels)
